                    ...
                    if (filter.getField().contains(".")) {
                        ...
                        switch (filter.getOperator()) {
                            ...
                            case SearchFilterItem.OPERATOR_NOT_EQUAL_BOOL:
                                ...
                                break;
                            case SearchFilterItem.OPERATOR_MASTER_FILTER:
                                if (filter.getField().equals("dashboard") && !filter.getValue().isEmpty()) {
                                    predicates.add(
                                        criteriaBuilder.or(
                                            criteriaBuilder.like(
                                                criteriaBuilder.function(
                                                    "UPPER",
                                                    String.class,
                                                    root.join("userData", JoinType.INNER).get("lastName")
                                                ),
                                                getGreekString(filter.getValue().toUpperCase() + "%")
                                            ),
                                            criteriaBuilder.like(
                                                root.join("userData", JoinType.INNER).get("vatNumber"),
                                                filter.getValue() + "%"
                                            ),
                                            criteriaBuilder.like(
                                                criteriaBuilder.function(
                                                    "UPPER",
                                                    String.class,
                                                    root.join("sector", JoinType.INNER).get("name")
                                                ),
                                                "%" + filter.getValue().toUpperCase() + "%"
                                            )
                                        )
                                    );
                                } else if (filter.getField().equals("examPrescription") && !filter.getValue().isEmpty()) {
                                    Predicate or = criteriaBuilder.or(
                                        criteriaBuilder.like(
                                            root.join("cvPatient", JoinType.LEFT).get("patientLastName"),
                                            getGreekString(filter.getValue().toUpperCase() + "%")
                                        ),
                                        criteriaBuilder.like(
                                            root.join("cvPatient", JoinType.LEFT).get("patientAmka"),
                                            filter.getValue() + "%"
                                        ),
                                        criteriaBuilder.like(
                                            root.join("examPrescription", JoinType.LEFT).get("barcode"),
                                            filter.getValue() + "%"
                                        ),
                                        criteriaBuilder.like(root.get("barcode"), filter.getValue() + "%")
                                    );
                                    //                                if (isNumeric(filter.getValue())) {
                                    //                                    or.getExpressions().add(criteriaBuilder.equal(root.get("cvPatient").get("id"), Long.parseLong(filter.getValue().replaceFirst("^0+(?!$)", ""))));
                                    //                                }
                                    predicates.add(or);
                                } else if (filter.getField().equals("labTest") && !filter.getValue().isEmpty()) {
                                    Predicate or = criteriaBuilder.or(
                                        criteriaBuilder.like(
                                            root.join("patient", JoinType.LEFT).get("patientLastName"),
                                            getGreekString(filter.getValue().toUpperCase() + "%")
                                        ),
                                        criteriaBuilder.like(
                                            root.join("patient", JoinType.LEFT).get("patientAmka"),
                                            filter.getValue() + "%"
                                        ),
                                        criteriaBuilder.like(
                                            root.join("patient", JoinType.LEFT).get("identificationNumber"),
                                            filter.getValue() + "%"
                                        ),
                                        criteriaBuilder.like(
                                            root.join("cvExamPrescription", JoinType.LEFT).get("barcode"),
                                            filter.getValue() + "%"
                                        )
                                    );
                                    //                                if (isNumeric(filter.getValue())) {
                                    ...
                                    //                                }
                                    predicates.add(or);
                                } else if (filter.getField().equals("cvSampleCheck") && !filter.getValue().isEmpty()) {
                                    predicates.add(
                                        criteriaBuilder.or(
                                            criteriaBuilder.like(
                                                root.join("citizen", JoinType.LEFT).get("lastName"),
                                                getGreekString(filter.getValue().toUpperCase() + "%")
                                            ),
                                            criteriaBuilder.like(
                                                criteriaBuilder.upper(root.join("citizen", JoinType.LEFT).get("lastName")),
                                                filter.getValue().toUpperCase() + "%"
                                            ),
                                            criteriaBuilder.like(root.join("citizen", JoinType.LEFT).get("amka"), filter.getValue() + "%"),
                                            criteriaBuilder.like(
                                                root.join("citizen", JoinType.LEFT).get("identificationNumber"),
                                                filter.getValue() + "%"
                                            ),
                                            criteriaBuilder.like(root.get("barcode"), filter.getValue() + "%"),
                                            criteriaBuilder.like(
                                                root.join("plf", JoinType.LEFT).get("uniqueCode"),
                                                "%" + filter.getValue() + "%"
                                            ),
                                            criteriaBuilder.like(root.get("plfUniqueCode"), "%" + filter.getValue() + "%")
                                        )
                                    );
                                } else if (filter.getField().equals("plf") && !filter.getValue().isEmpty()) {
                                    predicates.add(
                                        criteriaBuilder.or(
                                            criteriaBuilder.like(root.get("uniqueCode"), filter.getValue() + "%"),
                                            criteriaBuilder.like(
                                                root.join("personal", JoinType.LEFT).get("lastName"),
                                                getGreekString(filter.getValue().toUpperCase() + "%")
                                            ),
                                            criteriaBuilder.like(
                                                criteriaBuilder.upper(root.join("personal", JoinType.LEFT).get("lastName")),
                                                filter.getValue().toUpperCase() + "%"
                                            ),
                                            criteriaBuilder.like(
                                                root.join("personal", JoinType.LEFT).get("firstName"),
                                                getGreekString(filter.getValue().toUpperCase() + "%")
                                            ),
                                            criteriaBuilder.like(
                                                criteriaBuilder.upper(root.join("personal", JoinType.LEFT).get("firstName")),
                                                filter.getValue().toUpperCase() + "%"
                                            )
                                        )
                                    );
                                ...
                                break;
                            case SearchFilterItem.OPERATOR_ADDRESS_SEARCH:
                                if (filter.getField().equals("masterFilter")) {
                                    Expression<String> postalCode = root.get("zip");
                                    Expression<String> city = root.get("districtName");
                                    Expression<String> street = root.get("street");

                                    filter.setValue(filter.getValue().replace(',', ' '));
                                    filter.setValue(filter.getValue().trim().replaceAll(" +", " "));

                                    //All possible combinations (street might be null)
                                    Expression<String> exp1 = concat(criteriaBuilder, " ", postalCode, city, street);
                                    Expression<String> exp2 = concat(criteriaBuilder, " ", postalCode, street, city);
                                    Expression<String> exp3 = concat(criteriaBuilder, " ", city, postalCode, street);
                                    Expression<String> exp4 = concat(criteriaBuilder, " ", city, street, postalCode);
                                    Expression<String> exp5 = concat(criteriaBuilder, " ", street, city, postalCode);
                                    Expression<String> exp6 = concat(criteriaBuilder, " ", street, postalCode, city);
                                    Expression<String> exp7 = concat(criteriaBuilder, " ", city, postalCode);
                                    Expression<String> exp8 = concat(criteriaBuilder, " ", postalCode, city);

                                    predicates.add(
                                        criteriaBuilder.or(
                                            criteriaBuilder.like(exp1, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp2, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp3, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp4, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp5, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp6, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp7, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%")),
                                            criteriaBuilder.like(exp8, CustomUtil.stripAccents(filter.getValue().toUpperCase() + "%"))
                                        )
                                    );
                                }
                                break;
