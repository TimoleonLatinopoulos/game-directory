...
/**
 * REST controller for managing {@link com.timoleon.gamedirectory.domain.Comment}.
 */
@RestController
@RequestMapping("/api")
public class CommentResource {
    ...
    /**
     * {@code GET  /comments/game/:id} : get the comments with the game "id" and not current user.
     *
     * @param id the id of the game for the comments to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the comment, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/comments/game/{id}")
    public List<Comment> getGameComments(@PathVariable Long id) {
        log.debug("REST request to get Comment by game id: {}", id);
        List<Comment> commentList = commentService.findByGameAndNotCurrentUser(id);
        return commentList;
    }

    /**
     * {@code GET  /comments/user/game/:id} : get comment with the game "id" of the current user.
     *
     * @param id the id of the game for the comments to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the comment, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/comments/user/game/{id}")
    public List<Comment> getUserGameComment(@PathVariable Long id) {
        log.debug("REST request to get Comment by game id: {}", id);
        return commentService.findByGameAndCurrentUser(id);
    }
