...
type RestOf<T extends IComment> = Omit<T, 'datePosted'> & {
...
export class CommentService {
  ...
  findUserCommentByGame(id: number): Observable<HttpResponse<{}>> {
    return this.http.get<RestComment>(`${this.resourceUrl}/user/game/${id}`, { observe: 'response' });
  }

  findAllByGame(id: number): Observable<HttpResponse<{}>> {
    return this.http.get<RestComment>(`${this.resourceUrl}/game/${id}`, { observe: 'response' });
  }
  ...
  protected convertDateFromClient<T extends IComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      datePosted: comment.datePosted?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      datePosted: restComment.datePosted ? dayjs(restComment.datePosted) : undefined,
    };
  }
